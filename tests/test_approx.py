...
def eval_pert(sigma_v: float, e: float, a: float, m1: float, m2: float):
    ...
    args = (rand_params['v_infty'], rand_params['b'],
            rand_params['Omega'], rand_params['inc'], rand_params['omega'],
            e, a, m1, m2, rand_params['m3'])

    tidal_param = model_utils.tidal_param(rand_params['v_infty'], rand_params['b'], a, m1, m2)
    slow_param = model_utils.slow_param(rand_params['v_infty'], rand_params['b'], a, m1, m2)

    de_sim = np.abs(model_utils.de_SIM_rand_phase(*args)[0])
    de_hr = np.abs(model_utils.de_HR(*args))
...
def get_approx(sigma_v: float, num_systems: int):
    ...
    df.to_parquet(f'test_data/test_approx_{sigma_v}.pq', engine='pyarrow')

def thresh_mean(arr):
    return np.mean(arr) if arr.shape[0] > 5 else np.nan

def test_approx(sigma_v=1.266, num_systems=500000):
    path = f'test_data/test_approx_{sigma_v}.pq'
    ...
    ret = binned_statistic_2d(df['tidal_param'], df['slow_param'], np.log10(df['err']), statistic=thresh_mean, bins=[t_bins, s_bins])

    fig, axs = plt.subplots(1, 2, gridspec_kw={"width_ratios": [1, 0.05]})
    fig.set_size_inches(2.5, 2)
    fig.subplots_adjust(wspace=0.25)

    img = axs[0].imshow(ret.statistic.T, origin='lower', extent=(0, max_t, 0, max_s), cmap='RdYlBu_r', aspect=max_t / max_s, vmin=-2, vmax=0.5)
    axs[0].set_xlabel('$T$')
    axs[0].set_ylabel('$S$')
    ...
    plt.savefig(f'test_data/test_approx_{sigma_v}.pdf', format='pdf', bbox_inches="tight")
