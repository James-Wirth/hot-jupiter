...
class Cluster:
    ...
    def get_number_density(self, r: float, t: float): return self.profile.get_number_density(r, t)
    def get_isotropic_velocity_dispersion(self, r: float, t: float): return self.profile.get_isotropic_velocity_dispersion(r, t)
    ...
    def get_radius(self, lagrange: float, t: float):
        if not (0 < lagrange < 1):
            raise ValueError("Lagrange mass fraction must be between 0 and 1 (exclusive)")
    ...
    def get_lagrange_distribution(self, n_samples: int, t: float, seed=None):
        ...
        if not (0 < cdf_max <= 1):
            raise ValueError(f"Invalid CDF value {cdf_max:.3f} returned for r_max={self.r_max} at time t={t}")
