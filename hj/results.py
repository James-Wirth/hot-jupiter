...
def _to_code(label: LabelOrEnum) -> int:
    """LabelOrEnum -> StopCode integer value."""
...
def _codes(labels: Iterable[LabelOrEnum]) -> list[int]:
    """Sequence of LabelOrEnum -> list of StopCode integer values."""
    return [_to_code(lbl) for lbl in labels]


class Results:
    """Filter, summarize, and plot HJModel simulation outcomes."""
    ...
    def filter_outcomes(
        ...
        """Filter by outcome label(s), radius range, and per-outcome sampling fraction."""
        df = self.df
        if include:
    ...
    def _iter_outcome_slices(
        ...
        """Yield (code, hex_color, label_name, sub_df) for each outcome in `outcomes`."""
    ...
    def plot_stopping_cdf(
        ...
        """Cumulative distribution of stopping times by outcome."""
    ...
    def plot_sma_distribution(
        ...
        """Semi-major axis distribution per outcome."""
    ...
    def plot_sma_scatter(
        ...
        """Scatter of semi-major axis vs cluster radius per outcome."""
        df = self.filter_outcomes(
            include=list(conditions), sample_frac={"NM": 1 - drop_frac}
        )
    ...
    def compute_outcome_probabilities(
        ...
        """P(outcome) for each StopCode, optionally within a radius window."""
        df = self.df
    ...
    def get_statistics_for_outcomes(
        ...
        """Values of `feature` for systems matching `outcomes` within r <= r_max."""
    ...
    def project_radius(self, random_seed: int | None = None) -> None:
        """Project 3D radius onto a random 2D plane; adds 'r_proj' column to self.df."""
    ...
    def radius_histogram(
        ...
        """P(outcome | r) binned in `label`; bins below min_counts are dropped."""
    ...
    def plot_projected_probability(
        ...
        """P(outcome) as a function of projected radius r_proj."""
        self.project_radius()
