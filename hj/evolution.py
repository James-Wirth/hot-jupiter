...
def sample_initial_conditions(
    ...
    """Sample n initial systems (Lagrange position, e, a, m1, m2) from the cluster IC priors."""
    state = State.empty(n)
...
def _nbody_one(args: tuple[float, ...]) -> tuple[float, float]:
    """Worker entry point: unpack 11-float args, return (delta_e, delta_a)."""
...
def _batch_nbody(
    ...
    """Dispatch the per-step N-body subset; mean-anomaly is drawn here."""
    m = idx.size
...
def run_simulation(
    ...
    """Lockstep vectorized evolution; mutates `state` in-place until every system has stopped."""
    n = len(state)
