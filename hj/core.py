...
def _solve_kepler(mean_anom: float, e: float) -> float:
    """Solve Kepler's equation M = E - e sin E for E (Newton + cubic correction)."""
...
def _convert_mean_to_true_anomaly(mean_anom: float, e: float) -> float:
    """Mean -> true anomaly via eccentric anomaly."""
...
def _integration_time(
    ...
    """Encounter integration window 2 t(theta_crit) on the hyperbolic orbit."""
...
def _analytic_encounter_de(
    ...
    """delta_e from a single encounter (Heggie & Rasio 1996); returns (valid, delta_e)."""
...
def _sample_m3(u: float) -> float:
    """Perturber mass — 47 Tuc IMF inverse-CDF (Giersz & Heggie 2011)."""
    if u <= _M3_Y_CRIT:
...
def _tidal_f_factor(e: float) -> float:
    """f(e) — eccentricity enhancement in tidal dissipation (Hut 1981)."""
    e2 = e * e
    e4 = e2 * e2
    e6 = e4 * e2
    e8 = e4 * e4
...
def _tidal_derivs(e: float, a: float, m1: float, m2: float) -> tuple[float, float]:
    """(de/dt, da/dt) from equilibrium-tide circularisation (Hut 1981)."""
    mass_ratio = m2 / m1
...
def _apply_tidal(
    ...
    """Adaptive RK2 (midpoint) tidal evolution over dt_total."""
    step_factor = 0.05
...
def critical_radii(
    ...
    """Critical radii (R_td, R_hj, R_wj) for stop classification."""
...
def _classify_stop(
    ...
    """Return stop code or -1 if system is still active."""
    if e >= 1.0:
        return STOP_ION
    if a * (1.0 - e) < R_td:
        return STOP_TD
    ...
    if R_hj < a < R_wj and (cir or timeout):
        return STOP_WJ
    if cir or timeout:
        # At simulation end, a system that hasn't ionized, tidally disrupted,
        # circularized into HJ/WJ, or migrated to a < R_wj is classified NM.
        return STOP_NM
    return -1
...
def _plummer_radius_and_env(
    ...
    """(r, n_tot, sigma_v) at Lagrangian fraction `lagrange`, time t."""
...
def step(
    ...
    For each system:
      ...
      (1) Plummer eval at the system's current t.
      (2) Sample waiting time, advance t (capped at time_total), apply tidal RK2.
      (3) Stop check.
      (4) Sample one encounter from pre-drawn randoms; try analytic.
      (5) If analytic valid (or hybrid_switch=False): commit delta_e, stop check.
...
def recheck_stop(
    ...
    """Re-evaluate stop conditions for systems whose state was updated by N-body."""
    M = idx.shape[0]
    for k in prange(M):
        i = idx[k]
        code = _classify_stop(
...
def nbody_encounter_de(
    ...
    """REBOUND IAS15 integration of one 3-body encounter; returns (delta_e, delta_a)."""
    a_pert, e_pert, r_p = _perturber_orbit(v_infty, b, m1, m2)
    t_int = _integration_time(a_pert, e_pert, r_p, m1, m2)
...
def plummer_kernel_params(plummer) -> np.ndarray:
    """Pack Plummer attributes into the 7-float array consumed by the njit step kernel."""
    return np.array(
        [
            plummer.R0,
            plummer.A,
